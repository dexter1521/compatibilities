        ...
        <tr x-show="busq === '' || '<?= strtolower(esc($m['marca_nombre'])) ?> <?= strtolower(esc($m['modelo'])) ?>'.includes(busq.toLowerCase())">
            ...
            <td style="text-align:right;white-space:nowrap;">
                <button
                    class="btn-tbl-edit"
                    title="Aliases"
                    hx-get="<?= site_url('/motos/' . $m['id'] . '/aliases') ?>"
                    hx-target="#modal-content"
                    hx-swap="innerHTML"
                    style="background:rgba(249,115,22,.09);color:var(--compat-accent);">
                    <i class='bx bx-purchase-tag'></i></button>
                <button
                    class="btn-tbl-edit"
                    title="Editar"
                ...
                <button
                    ...
                    hx-swap="innerHTML"
                    hx-confirm="¿Eliminar la moto '<?= esc($m['marca_nombre']) ?> <?= esc($m['modelo']) ?>'?"><i class='bx bx-trash'></i></button>
            </td>
        </tr>
    <?php endforeach ?>
<?php endif ?>